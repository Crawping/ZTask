...
dump_c_mem() {
    ...
    for (i = 0; i < SLOT_SIZE; i++) {
        ...
        if (data->handle != 0 && data->allocated != 0) {
            ...
            ztask_error(NULL, "0x%x -> %zdkb", data->handle, data->allocated >> 10);
        }
    }
...
void *
ztask_lalloc(void *ptr, size_t osize, size_t nsize) {
    if (nsize == 0) {
        ztask_free(ptr);
        return NULL;
    }
...
void
ztask_debug_memory(const char *info) {
    // for debug use
    uint32_t handle = ztask_current_handle();
