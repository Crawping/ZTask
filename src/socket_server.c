...
#define MALLOC ztask_malloc
#define FREE ztask_free
...
static inline void
free_request_package(struct ztask_message_cmd *msg, void *u) {
...
cmd_cb(uv_async_t* handle) {
    ...
    while (true)
    {
        ...
        // 取出task_client首地址
        struct ztask_message_cmd *w = QUEUE_DATA(q, struct ztask_message_cmd, wq);
...
send_request(struct socket_server *ss, struct request_package *request, char type, int len) {
    ...
    struct ztask_message_cmd *msg = MALLOC(sizeof(*msg));
