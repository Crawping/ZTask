...
save_uncomplete(lua_State *L, int fd) {
	struct queue *q = get_queue(L);
	int h = hash_fd(fd);
	struct uncomplete * uc = ztask_malloc(sizeof(struct uncomplete));
...
push_more(lua_State *L, int fd, uint8_t *buffer, int size) {
	...
	if (size < pack_size) {
		...
		uc->pack.buffer = ztask_malloc(pack_size);
...
filter_data_(lua_State *L, int fd, uint8_t * buffer, int size) {
	...
	if (uc) {
		...
		if (size == pack_size) {
			...
			lua_pushvalue(L, lua_upvalueindex(TYPE_DATA));
			lua_pushinteger(L, fd);
			void * result = ztask_malloc(pack_size);
...
static int
lfilter(lua_State *L) {
	struct ztask_socket_message *message = lua_touserdata(L,2);
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = ztask_malloc(len + 2);
...
LUAMOD_API int luaopen_netpack(lua_State *L) {
    luaL_requiref(L, "netpack", luaopen_netpack_, 1);
