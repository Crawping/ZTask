...
pack(lua_State *L, void *data, size_t size) {
	struct mc_package * pack = ztask_malloc(sizeof(struct mc_package));
	pack->reference = 0;
	pack->size = (uint32_t)size;
	pack->data = data;
	struct mc_package ** ret = ztask_malloc(sizeof(*ret));
	*ret = pack;
...
LUAMOD_API int
luaopen_multicast_core_(lua_State *L) {
	luaL_Reg l[] = {
		{ "pack", mc_packlocal },
...
LUAMOD_API int luaopen_multicast_core(lua_State *L) {
    luaL_requiref(L, "multicast.core", luaopen_multicast_core_, 1);
