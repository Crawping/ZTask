...
lpackresponse(lua_State *L) {
	...
	// and the msg/sz is return by ztask.rawcall , so don't free(msg)
...
LUAMOD_API int
luaopen_cluster_core_(lua_State *L) {
	luaL_Reg l[] = {
...
LUAMOD_API int luaopen_cluster_core(lua_State *L) {
    luaL_requiref(L, "cluster.core", luaopen_cluster_core_, 1);
