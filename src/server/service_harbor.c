...
push_queue_msg(struct harbor_msg_queue * queue, struct harbor_msg * m) {
    ...
    if (((queue->tail + 1) % queue->size) == queue->head) {
        struct harbor_msg * new_buffer = ztask_malloc(queue->size * 2 * sizeof(struct harbor_msg));
        int i;
        for (i = 0; i < queue->size - 1; i++) {
            new_buffer[i] = queue->data[(i + queue->head) % queue->size];
        }
        ztask_free(queue->data);
...
new_queue() {
    struct harbor_msg_queue * queue = ztask_malloc(sizeof(*queue));
    queue->size = DEFAULT_QUEUE_SIZE;
    queue->head = 0;
    queue->tail = 0;
    queue->data = ztask_malloc(DEFAULT_QUEUE_SIZE * sizeof(struct harbor_msg));
...
hash_insert(struct hashmap * hash, const char name[GLOBALNAME_LENGTH]) {
    ...
    struct keyvalue * node = ztask_malloc(sizeof(*node));
...
hash_new() {
    struct hashmap * h = ztask_malloc(sizeof(struct hashmap));
    memset(h, 0, sizeof(*h));
    return h;
}
...
report_harbor_down(struct harbor *h, int id) {
    ...
    ztask_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, down, n);
}

struct harbor *
    harbor_create(void) {
    struct harbor * h = ztask_malloc(sizeof(*h));
    memset(h, 0, sizeof(*h));
    h->map = hash_new();
    return h;
}
...
harbor_release(struct harbor *h) {
    ...
    for (i = 1; i < REMOTE_MAX; i++) {
        ...
        if (s->fd && s->status != STATUS_DOWN) {
            ...
            // never call ztask_send during module exit, because of dead lock
        }
    }
    hash_delete(h->map);
    ztask_free(h);
...
forward_local_messsage(struct harbor *h, void *msg, int sz) {
    ...
    if (ztask_send(h->ctx, header.source, destination, type | PTYPE_TAG_DONTCOPY, (int)header.session, (void *)msg, sz - HEADER_COOKIE_LENGTH) < 0) {
        if (type != PTYPE_ERROR) {
            // don't need report error when type is error
            ztask_send(h->ctx, destination, header.source, PTYPE_ERROR, (int)header.session, NULL, 0);
        }
        ztask_error(h->ctx, "Unknown destination :%x from :%x type(%d)", destination, header.source, type);
    }
}

static void
send_remote(struct ztask_context * ctx, int fd, const char * buffer, size_t sz, struct remote_message_header * cookie) {
    size_t sz_header = sz + sizeof(*cookie);
    if (sz_header > UINT32_MAX) {
        ztask_error(ctx, "remote message from :%08x to :%08x is too large.", cookie->source, cookie->destination);
        return;
    }
    uint8_t * sendbuf = ztask_malloc(sz_header + 4);
    ...
    //ztask_socket_send(ctx, fd, sendbuf, sz_header+4);
}
...
dispatch_name_queue(struct harbor *h, struct keyvalue * node) {
    ...
    struct ztask_context * context = h->ctx;
    ...
    if (fd == 0) {
        if (s->status == STATUS_DOWN) {
            ...
            ztask_error(context, "Drop message to %s (in harbor %d)", tmp, harbor_id);
        }
        else {
            if (s->queue == NULL) {
                s->queue = node->queue;
            ...
            if (harbor_id == (h->slave >> HANDLE_REMOTE_SHIFT)) {
                ...
                while ((m = pop_queue(s->queue)) != NULL) {
                    ...
                    ztask_send(context, m->header.source, handle, type | PTYPE_TAG_DONTCOPY, m->header.session, m->buffer, m->size);
                }
                release_queue(s->queue);
                s->queue = NULL;
            }
...
static void
push_socket_data(struct harbor *h, const struct ztask_socket_message * message) {
    //assert(message->type == ZTASK_SOCKET_TYPE_DATA);
    //int fd = message->id;
    //int i;
    //int id = 0;
    //struct slave * s = NULL;
    ...
    //}
    //if (s == NULL) {
    //	ztask_free(message->buffer);
    //	ztask_error(h->ctx, "Invalid socket fd (%d) data", fd);
    //	return;
    //}
    ...
    //		if (remote_id != id) {
    //			ztask_error(h->ctx, "Invalid shakehand id (%d) from fd = %d , harbor = %d", id, fd, remote_id);
    //			close_harbor(h,id);
    //			return;
    //		}
    //		++buffer;
    ...
    //			if (s->size[0] != 0) {
    //				ztask_error(h->ctx, "Message is too long from harbor %d", id);
    //				close_harbor(h,id);
    //				return;
    //			}
    //			s->length = s->size[1] << 16 | s->size[2] << 8 | s->size[3];
    //			s->read = 0;
    //			s->recv_buffer = ztask_malloc(s->length);
...
remote_send_handle(struct harbor *h, uint32_t source, uint32_t destination, int type, int session, const char * msg, size_t sz) {
    ...
    if (harbor_id == h->id) {
        // local message
        ztask_send(context, source, destination, type | PTYPE_TAG_DONTCOPY, session, (void *)msg, sz);
        return 1;
    }
    ...
    if (s->fd == 0 || s->status == STATUS_HANDSHAKE) {
        if (s->status == STATUS_DOWN) {
            ...
            ztask_send(context, destination, source, PTYPE_ERROR, 0, NULL, 0);
            ztask_error(context, "Drop message to harbor %d from %x to %x (session = %d, msgsz = %d)", harbor_id, source, destination, session, (int)sz);
        }
        else {
            if (s->queue == NULL) {
                s->queue = new_queue();
...
remote_send_name(struct harbor *h, uint32_t source, const char name[GLOBALNAME_LENGTH], int type, int session, const char * msg, size_t sz) {
    ...
    if (node->value == 0) {
        ...
        ztask_send(h->ctx, 0, h->slave, PTYPE_TEXT, 0, query, strlen(query));
        return 1;
    }
    else {
...
handshake(struct harbor *h, int id) {
    struct slave *s = &h->s[id];
    uint8_t * handshake = ztask_malloc(1);
    handshake[0] = (uint8_t)h->id;
    //ztask_socket_send(h->ctx, s->fd, handshake, 1);
}
...
harbor_command(struct harbor * h, const char * msg, size_t sz, int session, uint32_t source) {
    ...
    case 'N': {
        if (s <= 0 || s >= GLOBALNAME_LENGTH) {
            ztask_error(h->ctx, "Invalid global name %s", name);
            return;
        }
    ...
    case 'S':
    case 'A': {
        char *buffer = alloca(s + 1);
        memcpy(buffer, name, s);
        buffer[s] = 0;
        int fd = 0, id = 0;
        sscanf(buffer, "%d %d", &fd, &id);
        if (fd == 0 || id <= 0 || id >= REMOTE_MAX) {
            ztask_error(h->ctx, "Invalid command %c %s", msg[0], buffer);
            return;
        }
        struct slave * slave = &h->s[id];
        if (slave->fd != 0) {
            ztask_error(h->ctx, "Harbor %d alreay exist", id);
            return;
        }
        slave->fd = fd;

        //ztask_socket_start(h->ctx, fd);
        handshake(h, id);
        if (msg[0] == 'S') {
    ...
    default:
        ztask_error(h->ctx, "Unknown command %s", msg);
        return;
    }
}
...
static int
mainloop(struct ztask_context * context, void * ud, int type, int session, uint32_t source, const void * msg, size_t sz) {
    struct harbor * h = ud;
    switch (type) {
        //case PTYPE_SOCKET: {
        //	const struct ztask_socket_message * message = msg;
        //	switch(message->type) {
        //	case ZTASK_SOCKET_TYPE_DATA:
        //		push_socket_data(h, message);
        //		ztask_free(message->buffer);
        //		break;
        //	case ZTASK_SOCKET_TYPE_ERROR:
        //	case ZTASK_SOCKET_TYPE_CLOSE: {
        //		int id = harbor_id(h, message->id);
        //		if (id) {
        //			report_harbor_down(h,id);
        //		} else {
        //			ztask_error(context, "Unkown fd (%d) closed", message->id);
        //		}
        //		break;
        //	}
        //	case ZTASK_SOCKET_TYPE_CONNECT:
        //		// fd forward to this service
        //		break;
        //	case ZTASK_SOCKET_TYPE_WARNING: {
        //		int id = harbor_id(h, message->id);
        //		if (id) {
        //			ztask_error(context, "message havn't send to Harbor (%d) reach %d K", id, message->ud);
        //		}
        //		break;
        //	}
        //	default:
        //		ztask_error(context, "recv invalid socket message type %d", type);
        //		break;
        //	}
        //	return 0;
        //}
...
harbor_init(struct harbor *h, struct ztask_context *ctx, const char * args, const size_t sz) {
    ...
    h->id = harbor_id;
    h->slave = slave;
    ztask_callback(ctx, h, mainloop);
    ztask_harbor_start(ctx);
